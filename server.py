...
def bypass_cloudflare(url: str, retries: int, log: bool, islogin: bool, proxy: str = None) -> ChromiumPage:
...
async def get_cookies(url: str, islogin: bool = False, retries: int = 5, proxy: str = None):
    if not is_safe_url(url):
        raise HTTPException(status_code=400, detail="Invalid URL")
    try:
        driver = bypass_cloudflare(url, retries, log, bool(islogin), proxy)
